(() => {
    var __webpack_modules__ = {
        351: function(e, t, r) {
            ...
            var n = this && this.__importStar || function(e) {
                ...
                if (e != null)
                    for (var r in e)
                        if (r !== "default" && Object.hasOwnProperty.call(e, r)) s(t, e, r);
                o(t, e);
                return t
            };
        ...
        186: function(e, t, r) {
            ...
            t.getState = t.saveState = t.group = t.endGroup = t.startGroup = t.info = t.warning = t.error = t.debug = t.isDebug = t.setFailed = t.setCommandEcho = t.setOutput = t.getBooleanInput = t.getInput = t.addPath = t.setSecret = t.exportVariable = t.ExitCode = void 0;
            const a = r(351);
            const c = r(717);
            const u = r(278);
            const p = n(r(365));
            ...
            function exportVariable(e, t) {
                ...
                if (s) {
                    ...
                    const s = `${e}<<${t}${p.EOL}${r}${p.EOL}${t}`;
                    c.issueCommand("ENV", s)
                } else {
                    a.issueCommand("set-env", {
                        name: e
                    }, r)
                }
            }
        ...
        30: function(e, t, r) {
            ...
            t.GitHub = c.Octokit.plugin(u.restEndpointMethods, p.paginateRest).defaults(d);
        ...
        762: (e, t, r) => {
            ...
            class Octokit {
                constructor(e = {}) {
                    ...
                    r.headers["user-agent"] = [e.userAgent, `octokit-core.js/${c} ${s.getUserAgent()}`].filter(Boolean).join(" ");
                    if (e.baseUrl) {
                        r.baseUrl = e.baseUrl
                    }
                    if (e.previews) {
                ...
                return new fetch.Promise((function(r, n) {
                    ...
                    h.on("response", (function(e) {
                        ...
                        if (fetch.isRedirect(e.statusCode)) {
                            ...
                            switch (c.redirect) {
                                case "error":
                                    n(new FetchError(`uri requested responds with a redirect, redirect mode is set to error: ${c.url}`, "no-redirect"));
                                    finalize();
                                    return;
                                case "manual":
                                    if (o !== null) {
    ...
    (() => {
        const e = __nccwpck_require__(747);
        const t = __nccwpck_require__(622);
        const r = __nccwpck_require__(438);
        const s = __nccwpck_require__(186);
        const o = process.env.GITHUB_TOKEN;
        const n = r.getOctokit(o);
        async function run() {
            let o = t.join(process.env["GITHUB_WORKSPACE"], ".github", "labels.json");
            if (!s.getBooleanInput("delete")) {
                console.log("[Action] Will not delete any existing labels")
            }
            let i = await getCurrentLabels();
            let a = JSON.parse(e.readFileSync(o).toString());
            a.forEach((e => {
                if (e.color[0] === "#") {
                    e.color = e.color.slice(1)
                }
            }));
            let c = diffLabels(i, a);
            c.forEach((async e => {
                if (e.type === "create") {
                    let t = {
                        ...r.context.repo,
                    ...
                    console.log(`[Action] Creating Label: ${e.label.name}`);
                    await n.rest.issues.createLabel(t)
                } else if (e.type === "update") {
                    let t = {
                        ...r.context.repo,
                        current_name: e.label.name,
                        color: e.label.color,
                        description: e.label.description
                    };
                    console.log(`[Action] Updating Label: ${e.label.name}`);
                    await n.rest.issues.updateLabel(t)
                    ...
                    if (s.getBooleanInput("delete")) {
                        ...
                        console.log(`[Action] Deleting Label: ${e.label.name}`);
                        await n.rest.issues.deleteLabel(t)
                    }
                }
            }))
        }
        async function getCurrentLabels() {
            let e = await n.rest.issues.listLabelsForRepo({
                ...r.context.repo
            });
            let t = e.data;
            return t
        ...
        function diffLabels(e, t) {
            let r = e.map((e => e.name.toLowerCase()));
            let s = t.map((e => e.name.toLowerCase()));
            let o = [];
            r.forEach((r => {
                if (s.includes(r)) {
                    const n = e.filter((e => e.name === r))[0];
                    const i = t.filter((e => e.name === r))[0];
                    if (n.color !== i.color || typeof i.description !== "undefined" && n.description !== i.description) {
                        o.push({
                            type: "update",
                            label: i
                        })
